...
export class CreateEditModalComponent {
  ...
  @Input() data: ScmReceiveMode; // primary key
  ...
  submitCreate() {
    ...
    if (this.validateForm.valid) {
      ...
      this.receivingModeServices.create(this.validateForm.getRawValue()).subscribe({
        next: (res: any) => {
          this.msg.remove(id)
  ...
  submitEdit() {
    if (this.validateForm.valid) {
      const id = this.msg.loading('Action in progress..', { nzAnimate: true }).messageId
      this.receivingModeServices.patch(this.data.id, this.validateForm.getRawValue()).subscribe({
        next: (res: any) => {
