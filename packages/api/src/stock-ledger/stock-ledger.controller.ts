...
import { AuditData, StockLedgerService } from './stock-ledger.service';
...
export class StockLedgerController {
  ...
  async create(
    @Request() request: Req,
    @Body() createStockLedgerDto: { data: CreateStockLedgerDto[] },
  ) {
    const data = await this.stockLedgerService.create(createStockLedgerDto);
    return {
  ...
  async findAll(
    ...
    const resData = {
      ...
      data: data.map(
        (res: Partial<StockLedgerEntity>) => new StockLedgerEntity(res),
      ),
    };
    if (!query.pagination) {
      return {
  ...
  async update(
    @Request() request: Req,
    @Param('id') id: string,
    @Body() updateStockLedgerDto: UpdateStockLedgerDto,
  ) {
