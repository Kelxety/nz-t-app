...
import { CustomSupplierDecorator, CustomSupplierDecoratorGet } from './decorators/supplier.decorator';
...
export class SupplierController {
  ...
  @Get()
  @CustomGlobalDecorator(Prisma.ScmSupplierScalarFieldEnum, true, SupplierEntity)
  ...
  async fulltextSearch(
    @Query() query: QueryT,
  ): Promise<ResponseT<SupplierEntity[]>> {
    const data = await this.supplierService.fullTextSearch({
      searchData: query.q,
      data: query.filteredObject ? JSON.parse(query.filteredObject) : {},
      page: Number(query.page),
      pageSize: Number(query.pageSize),
