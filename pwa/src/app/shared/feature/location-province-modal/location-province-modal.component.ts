...
import { LocationProvinceService, LocationRegionService } from '@app/shared/data-access/api';
...
@Component({
  selector: 'app-location-province-modal',
  templateUrl: './location-province-modal.component.html',
  styleUrls: ['./location-province-modal.component.less']
})
export class LocationProvinceModalComponent implements OnInit, AfterViewInit {
  ...
  constructor (
    ...
    private apiService: LocationProvinceService,
  ...
  getData() {
    this.apiService.get(this.id).subscribe({
      next: (res:any) => {
        this.dataModel = res;
        this.validateForm.patchValue({
          provinceCode: this.dataModel.provinceCode,
          provinceName: this.dataModel.provinceName,
          state: this.convertStatus(this.dataModel.state),
          region: this.dataModel.region
        })
